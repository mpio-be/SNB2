...
#' @export
fetch_data_v2.1 = function (con, box = NA, from = NA, to = NA) {
...
#' @export
fetch_ins_outs_v2.1 = function (x, tr_threshold = 5, safety_threshold = 600) {
...
#' @export
assign_direction_v2.1 = function(x, hardware_threshold = 0.05) {
...
#' @export
combine_front_v2.1 = function(x, threshold = 1.5){
  #NA transp to ''
...
#2. run function "fetch_data_v2.1" (for input information see help file of function fetch_data in SNB package)
#3. run function "events_v2.1"
#4. author: LS
#5. not uploading function available yet

#' @export
events_v2.1 = function(x) {
  x = fetch_ins_outs_v2.1(x, threshold)
  if (nrow(x) == 0) 
    return()
  x = assign_direction_v2.1(x)
  if (nrow(x) == 0) 
    return()
  x = concat_events_v2.1(x)
  if(nrow(x) == 0) 
    return()
  x = combine_front_v2.1(x)
  return(x)
  }  