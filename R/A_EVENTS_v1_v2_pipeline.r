...
eva = function(df, setTZ = "Etc/GMT-2",
               time_threshold_v1 = 2, max_distance_v1 = 16*60*60,
               tr_threshold_v2 = 5, cluster_events_threshold_v2 = 2, 
               max_distance_v2 = 16*60*60, cluster_fronts_threshold_v2 = 5, 
               no_front_v2 = NULL, N_cores = 50) {
  library(doParallel)
  cl = makePSOCKcluster(N_cores) # 50 cores
  registerDoParallel(cl)
  
  d = foreach(i = 1:nrow(df)) %dopar% {
    require(SNB2)
    require(fasttime)
    datetime_ <- LB <- id <- transp <- bv <- bout_length <- r_pk <- NULL
    con = dbcon()
