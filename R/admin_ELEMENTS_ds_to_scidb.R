...
#' @note 			The attribute of each \code{data.table} is another one-row \code{data.table} whith 3 columns:
...
update_bTables <- function(con, dat, ui = FALSE, demo = FALSE, db = getOption("snbDB_v2") ) {
	...
	if(ui) {
		bar = shiny::Progress$new(min = 1, max = length(dat)); on.exit(bar$close())
		} else
	...
	for(i in 1:length(dat) ) {
		...
		if(ui) bar$set(i, message = 'Uploading txt files' ,detail = paste("Box", box)) else
					setTxtProgressBar(bar, i)

		writeok = dbWriteTable(con, box, di, row.names = FALSE, append = TRUE)
		if(writeok)
		dbq( con,
			paste('UPDATE file_status set upload_status =',aa$upload_status, ',dt_loaded =' ,shQuote(Sys.time()),
							'where id =' , aa$id) )


		}

		shiny::HTML( paste(length(dat), 'files uploaded to DB') )
